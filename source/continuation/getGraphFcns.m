...
fcns.fLMinus = @(z,Lambda) (A + B*spdiags(2*z.^2+Lambda,0,nA,nA));
fcns.JL = @(z,Lambda)[zeros(nA) fcns.fLMinus(z,Lambda); -fcns.fLinMatrix(z,Lambda) zeros(nA)]; % [0 L-; -L+ 0]
fcns.fL1 = @(z,Lambda) (A + B*spdiags(fprime(z)/2+z.^2+Lambda,0,nA,nA));
fcns.fL2 = @(z,Lambda) (B*spdiags(fprime(z)/2-z.^2,0,nA,nA));
fcns.H = @(z,Lambda)1i*[fcns.fL1(z,Lambda) fcns.fL2(z,Lambda); -fcns.fL2(z,Lambda) -fcns.fL1(z,Lambda)];
...
% This is Fzz considered as a tensor that takes three arguments, essentially a cubic form
% This is used in (7.130) of Govaerts 2000 SIAM book
% Note the components (1:end-1) of phi1 and phi2 contain the function  and the end component 
% contains the frequency parameter Lambda
